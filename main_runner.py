...
<div id="budget-panel" style="margin-bottom:14px;background:#1a1a1a;border:1px solid #444;border-radius:6px;padding:10px 16px;display:flex;gap:24px;align-items:center;flex-wrap:wrap">
  ...
  <div>
    <div style="font-size:0.7em;color:#888;margin-bottom:1px">Następne zlecenie (100% equity)</div>
...
<div class="indicators-panel" style="margin-top:18px">
  ...
  <div style="overflow-x:auto">
    <table id="a2-variant-table" style="min-width:900px">
      <tr>
        <th>Wariant</th>
        <th title="Łączna liczba setupów">Ilość</th>
        <th title="Procent setupów które weszły w pozycję">% entry</th>
        <th title="Średnie deklarowane R:R setupów które weszły">Avg RR</th>
        <th title="% uruchomionych który dotarł do TP1 lub wyżej (TP1/TP2/TP1+BE/TP1+SL)">TP1 rate</th>
  ...
  <div style="overflow-x:auto">
    <table id="a2-daily-table" style="min-width:520px">
      <tr>
        ...
        <th title="Stopy">L</th>
        <th title="% uruchomionych który dotarł do TP1+">Win rate</th>
        <th title="Suma PnL dnia">Σ PnL $</th>
        <th title="Suma PnL gdyby cała pozycja wyszła na TP1 (dla SL = rzeczywisty PnL)">Σ TP1-only $</th>
      </tr>
      <tr id="a2-daily-loading"><td colspan="8" style="color:#888;text-align:center">ładowanie...</td></tr>
    </table>
  </div>
  ...
  <div id="bt-summary" style="overflow-x:auto;display:none">
    <table id="bt-table" style="min-width:820px">
      <tr>
        ...
        <th title="% wejść na snapshots gdzie wszystkie warianty odpaliły jednocześnie (porównywalna podstawa)">Entry%</th>
...
function renderA2VariantTable(rows) {{
  ...
  rows.forEach(function(r) {{
    ...
    [
      ...
      r.avg_rr          != null ? r.avg_rr.toFixed(2)             : '—',
...
function renderA2Daily(rows) {{
  ...
  rows.forEach(function(r) {{
    ...
    [
      ...
      if (i === 7 && r.total_tp1only_usd != null) td.style.color = pnlColor(r.total_tp1only_usd);
    }});
  }});
}}
        ...
        return [
            {
                ...
                "sum_pct_tp1":  round(float(m["sum_tp1only_pct"]), 1) if m.get("sum_tp1only_pct") else None,
                "sum_pct_tp12": round(float(m["sum_pnl_pct"]), 1) if m.get("sum_pnl_pct") else None,
                "daily_pct_tp1":  round(float(m["sum_tp1only_pct"]) / period_days_count, 1) if m.get("sum_tp1only_pct") else None,
                "daily_pct_tp12": round(float(m["sum_pnl_pct"]) / period_days_count, 1) if m.get("sum_pnl_pct") else None,
            }
            for m in (stats.get("by_model") or [])
        ]
